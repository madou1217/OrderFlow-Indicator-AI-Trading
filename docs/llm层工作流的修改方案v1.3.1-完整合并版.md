# LLM层工作流修改方案 v1.3.1（完整合并版）

基于 /data/docs/订单流交易员交易流程V1.md 的工作流，将原有的 stage1 scan + stage2 entry 架构重构为三层架构。

本文是 `v1.3.0` 与 `v1.3.1` 的完整合并版主文档：
- 不再要求读者先读旧稿，再读增量修订稿
- 若本文件与旧版文档冲突，以本文件为准
- 文档目标是提供一份可以直接继续实现、继续迭代的单一主版本
...
- v1.3.1：修复 v1.3.0 剩余的5个合同级缺口，并整理为完整合并版主文档

### v1.3.1 变更内容

1. **修复 invalid thesis 处理矛盾**：thesis_status=invalid 时严格禁止新开仓，删除越权示例；新增紧急刷新通道（Stage2 请求后 Stage1 在下一个 15m cycle 内响应，而非等 4H）
2. **补齐 `market_tradeable=false` 的重新进入闭环**：`recheck_conditions` 结构化，Stage2 每个 cycle 消费；命中后触发紧急刷新
3. **触发事件绑定新鲜度、位置与时序**：谓词库使用 `max_age_minutes`、`near_level`、`event_after_precondition`，确保确认信号发生在对的位置、对的时间、对的顺序
4. **1m/100ms 执行帧从 LLM 剥离并补全执行合同**：Stage2 输出完整 `execution_intent`（含 `intent_mode`、`ttl_minutes`、`max_drift_pct`），精细入场由独立的确定性执行引擎完成
5. **支持 paths=[] 和 `market_tradeable=false`**：Stage1 允许输出空 paths，价格在 value 中间时显式拒绝编故事
6. **thesis_premises 扩展为可变长度**：Stage1 根据剧本类型声明 3-8 条前提，覆盖 funding、VPIN、位置漂移、value acceptance 等维度
7. **补全路径质量合同**：`path_quality` 增加 `expected_entry_basis` / `quality_target_basis` / `planned_stop_loss` 等必填字段，Stage2 在生成 `execution_intent` 前按最终入场方式重算 RR
...
- **确认信号必须发生在对的位置、对的时间、对的顺序**——脱离位置、时间或当前这次互动顺序的确认是噪音
...
- **no-trade 地图也必须有重新进入闭环**——没有 edge 时可以不做，但 edge 出现时系统必须及时重画地图
...
2. **确认信号必须发生在对的位置、对的时间、对的顺序**：谓词绑定 `max_age_minutes + near_level + event_after_precondition`
3. **执行层有足够分辨率**：1m/100ms 执行由独立引擎完成，不受 LLM 响应延迟限制
4. **没有 edge 时允许明确不做，但必须能重新进入**：`market_tradeable=false` 时用 `recheck_conditions` 驱动紧急刷新
5. **开仓前已证明路径质量**：path_quality（RR、障碍密度、路径阻力）作为最终开仓门槛，并在 Stage2 以最终执行方式重算
...
indicator_engine (27个指标，含 i27 options_surface)
   ...
   输入: indicator_summary         输入: indicator_summary (最新)
         + previous_paths                + market_tradeable/recheck_conditions
   输出: market_tradeable                + stage1的paths
         + recheck_conditions            + stage1的thesis_premises
         + paths[]                       + entry_snapshots (如有持仓)
         + driver_attribution      输出: decision
         + thesis_premises               + execution_intent (如开仓)
...
**代码层数据合同**：所有事件（`divergence_events`、`absorption_events`、`exhaustion_events`、`initiation_events`）统一增加 `confirmed_price` 字段，确保 Stage2 可以评估事件发生的位置。`price_location` 增加 `at_edge` 和 `edge_type` 字段，代码层实时计算。
...
```json
{
  ...
  "refresh_reason": "scheduled_4h | emergency_thesis_invalid | emergency_premises_violated | recheck_condition_met",
...
Stage1 在画完地图后，先判断当前是否根本没有 edge 可做。如果没有，输出 `market_tradeable: false`，不构建 path，但必须同时给出结构化 `recheck_conditions`，供 Stage2 每个 15m cycle 检查是否需要紧急刷新地图。
...
- `ema_regime=between`：趋势结构不清晰
- `ratio/funding/vpin/oi_change` 没有给出 crowding、恐慌、扩张中的异常状态
...
输出0-2条path。每条包含七个价格化字段：
- thesis：当前主剧本是什么，为什么是它
- activation_level：哪个价格带被确认后剧本激活（结构化谓词，含新鲜度、位置与事件时序约束）
...
Stage2 收到 `market_tradeable = false` 时，不是简单空等到下一个 4H，而是每个 15m cycle 都先检查 `recheck_conditions`。任意一条命中，则请求 Stage1 紧急刷新。
...
9. **如果 path 有 precondition，必须显式声明 `event_after_precondition`**——确保确认属于"当前这次互动"，而不是旧事件误触发新 setup
10. **path_quality 必须为每条 path 评估路径质量**——包括 RR、障碍密度、路径阻力，且 RR 必须可由字段重算
...
| `requires.event_after_precondition` | trigger event 的 `confirmed_at` 必须晚于最近一次 precondition 满足时间 | `true/false` |
...
- `event_after_precondition`：解决"上一次 interaction 的确认误触发这一次 setup"的问题。只有当事件发生在最近一次 precondition 满足之后，才算这次 setup 的有效确认。
- 三个约束是 AND 关系：事件必须**既在时间窗口内，又在位置窗口内，并且发生在当前这次 precondition 之后**才算有效。
...
在做任何交易评估之前，Stage2 先检查 Stage1 当前给的地图是否还能被继续使用：

1. 检查 `market_tradeable`
   - 如果为 `false`，先逐条检查 `recheck_conditions`
   - 任意一条命中：`request_stage1_refresh = true`，`refresh_urgency = "recheck_condition_met"`，本轮仍然 `NO_TRADE`
   - 若没有命中：继续 `NO_TRADE`，仅做持仓管理
2. 若 `market_tradeable = true`，逐条检查 `thesis_premises`：对比 indicator_summary 中的实际值与 premise 的 expected 值
3. 评估整体 thesis 健康度：
   - `valid`：所有前提仍然成立
...
其中，若 path 声明了 `event_after_precondition = true`，Stage2 还必须补做：
- 找到最近一次 precondition 满足时间 `precondition_satisfied_at`
- 要求 `trigger_event.confirmed_at >= precondition_satisfied_at`

注意：path之间的切换可以在failure_level未被击穿时发生——当alternate path的activation条件被满足时（包括其 precondition、freshness、location、event ordering 检查全部通过），Stage2可以判定"primary path已经跑完了它的路径，alternate path的激活条件已满足"，这是合法的切换，不需要primary path先hit failure。**但前提是 thesis_status 不是 invalid**——如果 thesis 已经失效，任何基于该 thesis 的 path 切换都不允许。
...
- absorption 或 exhaustion 已确认？（必须满足 path 里声明的 `max_age_minutes`、`near_level`，若有 `event_after_precondition` 还要满足时序约束）
- 有效 divergence？（同上）
- spot 没有继续推同方向？
...
- **触发已确认**：上面的checklist通过，且所有事件满足 age、location、event ordering 约束
...
Gate 通过后，在生成 `execution_intent` 之前，Stage2 必须基于最终的执行方式重算路径质量：

1. 先确定最终 `expected_entry`
   - `intent_mode = immediate`：用当前可执行价附近重算
   - `intent_mode = pullback`：用 `entry_zone_mid`
   - `intent_mode = breakout`：用 breakout trigger price
2. 按以下公式重算：
...
3. **RR 比值**：`rr_ratio ≥ 1.5` 才允许开仓。如果 `1.0 ≤ rr_ratio < 1.5` 且其他条件极好，允许但必须降低仓位
4. **障碍密度**：`obstacles_to_target` 中的关键级别（HVN/POC/AVWAP/RVWAP中枢）超过 2 个 → 路径质量降级
5. **路径阻力综合判断**：
...
注意：Stage1 的 `path_quality` 是地图级粗估，Stage2 的重算结果才是最终是否允许开仓的依据。如果当前数据显示某个 obstacle 已不存在，Stage2 可以将其从 obstacles 中移除并重新评估 `path_resistance`。
...
```json
{
  ...
  "path_evaluation": {
    "path_b": {
      ...
      "activation_predicate": {
        ...
        "precondition_satisfied_at": "2026-03-26T18:21:00Z",
  ...
  "trigger_checklist": {
    "setup_type": "B_reversal",
    "absorption_or_exhaustion": { "pass": true, "detail": "selling_exhaustion at 2042.09, confirmed 18:25Z, age=5min (≤60min), distance=0.20% from 2035 (≤0.5%), occurred after latest precondition" },
  ...
  "hard_gate": {
    ...
    "trigger_confirmed": { "pass": true, "detail": "4/4 reversal checklist conditions met, all events within age, location, and ordering bounds" },
...
1. **识别意图模式**
   - `pullback`：等待价格回到 `entry_zone` 内
   - `breakout`：等待价格突破 `entry_zone` 的 trigger side
   - `immediate`：当前价已在 `entry_zone` 内，立即进入下单流程
2. **入场价优化**：在允许的执行场景内，用 `intrabar_poc` 作为 limit price 参考
3. **临门确认**：OFI 和 microprice 与 `side` 同向时才下单；如果反向，等待下一个 1m bar 重新检查
4. **下单方式**：优先 `limit order at intrabar_poc`，如果 `ttl_minutes` 内未成交，自动取消；是否转 market 由执行引擎单独配置，不由 LLM 决定
5. **止损/止盈**：下单后立即挂 `stop_loss` 和 `take_profit` orders
6. **取消规则**：满足任一条件即取消执行意图
   - 超过 `ttl_minutes`
   - 当前价相对 `zone_mid` 的偏离超过 `max_drift_pct`
   - `pullback` 模式下价格持续远离 `entry_zone` 且未回踩
   - `breakout` 模式下价格在 trigger 方向失效，重新回到 zone 的另一侧
   - Stage2 发出新的 intent 覆盖旧 intent
...
| 不做交易 | 不支持（总得给个方向） | `market_tradeable=false + paths=[]` 是合法输出，且有 `recheck_conditions` 重新进入闭环 |
| Condition格式 | 自然语言 | 结构化谓词 + `max_age_minutes` + `near_level` + `event_after_precondition` |
...
| 路径质量 | 无 | RR比值 + Stage2终值重算 + 障碍密度 + 路径阻力 → 仓位调节或拒绝 |
| 执行层 | LLM说"怎么进" | 执行引擎（代码，实时1m/100ms数据，支持 `intent_mode/ttl/max_drift`） |
...
| 时间(北京) | 价格 | 系统行为 |
|-----------|------|---------|
| 01:00 | 2063 | Stage1建立Path A(空延续) + Path B(reclaim反转) + thesis_premises(6条，含funding和位置前提) + path_quality(A: low resistance RR=2.45; B: medium resistance RR=2.15); market_tradeable=true |
| 01:45 | 2058→跌 | Stage2: thesis valid(6/6 pass), Path A activation谓词检查(15m close < 2050)尚未触发 |
| 02:00 | 2045 | Stage2: thesis valid, Path A activated(15m close < 2050), trigger check通过, hard gate通过(at_edge=true), path_quality终值重算通过(RR=2.45, resistance=low) → 输出execution_intent做空; 执行引擎: OFI同向, limit at intrabar_poc |
...
| 03:30 | 2050 | Stage2: 新thesis valid, 新Path activation谓词检查 — selling_exhaustion at 2042(age=65min ≤ 新path的max_age_minutes=90, near 2035 distance=0.20% ≤ 1.0%, `event_after_precondition=true`) → activated, trigger check通过, hard gate通过(at_edge=true, 4h RVWAP -1σ zone), path_quality终值重算通过(RR=2.15, resistance=medium) → 输出 `pullback` 模式 execution_intent 做多 |
...
| 时间点 | v1.2.0 行为 | v1.3.1 行为 | 差异原因 |
|--------|-----------|-----------|---------|
| 03:00 | thesis_status=weakened, 还在等 | thesis_status=invalid(critical p1 violated), 紧急刷新请求 | v1.3.x 区分 critical/supporting，OI翻转是 critical |
| 03:15 | — | Stage1紧急刷新，15min内响应 | v1.3.x 新增紧急刷新通道 |
| 03:30 | 按旧map的Path B开仓（越权） | 按新map的新Path开仓（合规） | invalid 时严禁用旧map，紧急刷新提供新map |
| 03:30 | exhaustion无age/location顺序检查 | exhaustion age/location/event ordering 全部通过后才有效 | v1.3.1 补上 `event_after_precondition` 时序约束 |
| 02:00 | 无路径质量检查 | path_quality 使用明确 RR 公式并按最终入场方式重算 | v1.3.1 补全 RR 合同和 Stage2 终值重算 |
...
5. 特别验证：`recheck_conditions` 的命中率与误触发率，确保 no-trade 地图既不会睡死，也不会过度刷新
