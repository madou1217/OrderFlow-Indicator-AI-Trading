...
pub fn workflow_user_prompt_prefix(stage: WorkflowPromptStage) -> &'static str {
    match stage {
        ...
        WorkflowPromptStage::Stage2A => {
            "You are a path auditor and tactical execution planner. Audit the current strategic path first, then output either PATH_CONFIRMED with tactical_entry_plan or REQUEST_STAGE1_REEVALUATION. Return only the workflow Stage2A JSON.\n\n"
        }
